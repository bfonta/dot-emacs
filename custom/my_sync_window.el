...
;;; my_sync_window --- Summary
;;; Code:
;;; Commentary: https://stackoverflow.com/questions/19987071/mirroring-location-in-file-in-two-opened-buffers-side-by-side
...
(defface sync-window-face ;; originally copied from font-lock-function-name-face
  '((((class color) (min-colors 88) (background light)) (:foreground "Yellow" :background "Blue1"))
    ...
    (t (:bold t)))
  "Face used to highlight regions in `my/sync-window-mode' slaves."
  :group 'sync-window)

(defvar sync-window-overlay nil
  "Overlay for current master region in `my/sync-window-mode' slaves.")
...
(defvar sync-window-mode-hook nil
  "Hooks to be run at start of `my/sync-window-mode'.")

(define-minor-mode my/sync-window-mode
  "Synchronized view of two buffers in two side-by-side windows."
  :group 'windows
  :lighter " ⇕"
  (if my/sync-window-mode
      (progn
...
only when the buffer of the active window is in `my/sync-window-mode'."
  (with-selected-window (or window (selected-window))
    (when my/sync-window-mode
      (sync-window display-start))))

(provide 'my_sync_window)
